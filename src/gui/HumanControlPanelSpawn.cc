// HumanControlPanelSpawn.cc -- spawn と削除。launch プロセスの起動、エンティティの生成/削除、
// 安全な spawn 位置のプロービングと、その確定待ち。
//
    ...
    double _x, double _y, double _z, double _yaw, int _attempt)
{
  ...
  if (IsActorIndex(modelIndex))
  {
    ...
    // サーバーが持っている状態を受け取る（構想書 §3・§13 段階3）。
    // トピック名は CharacterState.hh の StateTopicFor() で組み立てる
    // -- 送る側（ActorCommandPlugin）と同じ式を使うため、片方だけ変えて
    // 無言ですれ違うことがない。
    //
    // ラムダで名前を捕まえて添字は使わない。**添字は人物を削除するとずれる**
    // ので、非同期に届くコールバックが別人の行を書き換えうる（構想書 §6）。
    this->node.Subscribe<gz::msgs::Param>(
        StateTopicFor(nameStd),
        [this, nameStd](const gz::msgs::Param &_message)
        {
          this->OnCharacterState(nameStd, _message);
        });
