#ifndef GZ_HUMAN_SIM_HUMANCONTROLPANELINTERNAL_HH_
#define GZ_HUMAN_SIM_HUMANCONTROLPANELINTERNAL_HH_

// HumanControlPanel*.cc が共有する、ファイルスコープの表・調整用の定数・
// 小さな純粋関数。もとは 3808 行あった単一の HumanControlPanel.cc の
// 先頭に `static` で並んでいたもので、責務ごとにファイルを割った結果、
// 複数の .cc が同じものを見る必要が出たのでここへ移した。
//
// **`static` ではなく `inline` にしてある。** `static` のままだと
// 翻訳単位ごとに別の実体ができ、`static` な表を読む `static` な関数は
// .cc ごとに違うコピーを掴む（ODR 違反）。値と挙動は単一ファイル版と同じ。
//
// ここに置くのは「パネルの状態を持たない共有物」だけ。状態と振る舞いは
// HumanControlPanel.hh とその .cc 群の側にある。
...
#endif  // GZ_HUMAN_SIM_HUMANCONTROLPANELINTERNAL_HH_
