...
// SpawnSection -- 人物のspawn（モデル・ポーズ・動作モード・名前・配置方法）と、spawn済み一覧
//
...
ColumnLayout {
  ...
  // spawn フォームの選択状態。以前は root(Rectangle) 側にあり、ここからは
  // スコープ鎖をたどって見えていた。ファイルが分かれると見えないので、
  // 使っているのがこのセクションだけである以上、持ち主をこちらへ移す。
  ...
  // このセクションが持つ状態。**もとは root.placingByClick と書かれていたが、
  // 宣言は root(Rectangle) ではなく内側の ColumnLayout に付いていた**ので、
  // 読むと undefined、書くと "Cannot assign to non-existent property" に
  // なっていた（実測。QML はこれをエラーで止めず、その行以降の
  // onToggled ブロックが丸ごと実行されなくなるだけ）。結果、
  // 「3Dビューでクリック」を選んでも選択が定着せず setSpawnPicking(true) も
  // 呼ばれていない。下のコメントが言う single source of truth は
  // **効いていなかった**。持ち主をこのセクションにして修理する。
  ...
  Connections {
    ...
    function onSpawnPickingChanged() {
      placingByClick = HumanControlPanel.spawnPicking ||
    ...
    delegate: Rectangle {
      ...
      ColumnLayout {
        ...
        RowLayout {
          ...
          // サーバーが言っている状態（構想書 §3・§13 段階3）。
          // **GUI が「自分が送った指令」から推測した値ではない。**
          // まだ受信していない人物（静止モデルなど ActorCommandPlugin を
          // 持たないもの）は "-" のまま。
          Label {
            text: HumanControlPanel.humanStateLabel(index)
